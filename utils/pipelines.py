    ...
    def calculate(self):
        ...
        for (scheme_name, coding_scheme), needed_index in zip(self.coding_schemes.items(), self.needed_indices):
            for file_name, file_path in self.file_paths.items():
                ...
                pipeline = TranscodePipeline(coding_scheme=coding_scheme, error_correction=None,
                                             need_logs=self.need_logs)

                dna_sequences = pipeline.transcode(direction="t_c", input_path=file_path,
                                                   segment_length=self.segment_length, index=needed_index)["dna"]
