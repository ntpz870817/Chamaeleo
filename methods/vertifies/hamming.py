...
def add_for_matrix(matrix):
    ...
    for row in range(len(matrix)):
        verity_matrix.append(add_for_list(matrix[row], detect_site_length))

    return verity_matrix


def add_for_list(input_list, detect_site_length=None):
    """
...
def remove_for_matrix(verity_matrix):
    """
    ...
    for row in range(len(verity_matrix)):
        matrix.append(remove_for_list(verity_matrix[row]))
    return matrix


def remove_for_list(input_list):
    """
...
def verify_for_matrix(verity_matrix):
    """
    ...
    matrix = []
    for row in range(len(verity_matrix)):
        matrix.append(verify_for_list(verity_matrix[row], row))

    return matrix


# noinspection PyProtectedMember
def verify_for_list(input_list, row=None):
    """
    ...
    for parity in range(0, (len(output_list))):
        if 2 ** detect_site == parity + 1:
            start_index = 2 ** detect_site - 1
            index = start_index
            xor = []

            while index < len(output_list):
                block = output_list[index:index + 2 ** detect_site]
                xor.extend(block)
                index += 2 * 2 ** detect_site
